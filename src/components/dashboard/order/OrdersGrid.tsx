...
export function OrdersGrid({ orders, loading, expandedOrderId, pendingItems, pendingTables, ...actions }: Props) {
  ...
  return <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{orders.map(order => <OrderCard key={order.id} order={order} expanded={expandedOrderId === order.id} pendingItems={pendingItems} finishing={pendingTables.has(order.id)} onToggleExpand={actions.onToggleExpand} onManage={actions.onManage} onTogglePrepared={actions.onTogglePrepared} onFinish={actions.onFinish} />)}</div>;
}
